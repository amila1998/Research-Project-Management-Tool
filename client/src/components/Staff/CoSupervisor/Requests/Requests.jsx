...
const Requests = () => {
    ...
    useEffect(() => {
        const getMyRequests= async()=>{
            if (callback) {
                try {
                    setLoading(true)
                    const res = await axios.get(`/api/cosupervisor/getmygrouprequests`);
    ...
    try {
        const res = await axios.post(`/api/cosupervisor/giveResponse/${id}`,{
            cosupervisorResponse:true,
            level:6
        })
